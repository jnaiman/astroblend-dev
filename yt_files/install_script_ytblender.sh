...
# Here's where you put the Blender path if you like; otherwise it'll download it
# and install it on its own
INST_BLENDER=1 # install blender?
BLENDER_DIR="/Applications/Blender/blender.app/Contents/MacOS"

# Here's where you put the AstroBlend path if you like; otherwise it'll download it
# and install it on its own
INST_ASTROBLEND=0 # install AstroBlend?
ASTROBLEND_DIR="/Users/jillnaiman/astroblend-stable/science/"
...
function do_setup_py
{
    ...
    then
	( ${DEST_DIR}/bin/python3.4 setup.py build --hdf5=${HDF5_DIR} $* 2>&1 ) 1>> ${LOG_FILE} || do_exit
    else
        ( ${DEST_DIR}/bin/python3.4 setup.py build   $* 2>&1 ) 1>> ${LOG_FILE} || do_exit
    fi
    ( ${DEST_DIR}/bin/python3.4 setup.py install    2>&1 ) 1>> ${LOG_FILE} || do_exit
    touch done
    cd ..
}
...
echo '43adbef25e1b7a7a0be86231d4aa131d5aa3efd5608f572b3b0878520cd27054cc7993726f7ba4b5c878e0c4247fb443f367e0a786dec9fbfa7a25d67427f107  Python-3.4.1.tgz' > Python-3.4.1.tgz.sha512
...
then
    ...
    ( make install 2>&1 ) 1>> ${LOG_FILE} || do_exit
    ( ln -sf ${DEST_DIR}/bin/python$PYTHON_VER ${DEST_DIR}/bin/pyyt 2>&1 ) 1>> ${LOG_FILE}
    ( make clean 2>&1) 1>> ${LOG_FILE} || do_exit
    touch done
    cd ..
fi

export PYTHONPATH=${DEST_DIR}/lib/python$PYTHON_VER/site-packages/
...
( ${DEST_DIR}/bin/python$PYTHON_VER ${YT_DIR}/distribute_setup.py 2>&1 ) 1>> ${LOG_FILE} || do_exit

echo "Installing pip"
( ${DEST_DIR}/bin/easy_install-3.4 pip 2>&1 ) 1>> ${LOG_FILE} || do_exit
...
#emacs $DEST_DIR/src/yt-hg/yt/setup.py  #JPN
MY_PWD=`pwd`
#cd $YT_DIR
#( ${HG_EXEC} pull 2>1 && ${HG_EXEC} up -C 2>1 ${BRANCH} 2>&1 ) 1>> ${LOG_FILE}

echo "Installing yt...nearly"
[ $INST_PNG -eq 1 ] && echo $PNG_DIR > png.cfg
#( export PATH=$DEST_DIR/bin:$PATH ; ${DEST_DIR}/bin/python$PYTHON_VER setup.py develop 2>&1 ) 1>> ${LOG_FILE} #|| do_exit #JPN
#touch done
cd $MY_PWD

if !( ( ${DEST_DIR}/bin/python$PYTHON_VER -c "import readline" 2>&1 )>> ${LOG_FILE})
then
...
echo "Hey, you got this far, alright!"
echo "Now, lets update things to python 3"
cd $YT_DIR
#hg pull https://bitbucket.org/jnaiman/yt_python3
#hg up -C tip
#echo "[paths]" > .hg/hgrc
#echo "default = https://bitbucket.org/jnaiman/yt_python3" >> .hg/hgrc
${DEST_DIR}/bin/2to3-$PYTHON_VER *py
${DEST_DIR}/bin/python$PYTHON_VER setup.py install

cd $DEST_DIR


if [ $INST_ASTROBLEND -eq 1 ]
then
    if [ -z "$ASTROBLEND_DIR" ]
    then
	echo "ASTROBLEND_DIR is not set, but I haven't set up how to install AstroBlend yet!!"
	echo "exiting... sorry :("
	exit 1
    else
	echo "ASTROBLEND_DIR=${ASTROBLEND_DIR} , so I will not be installing AstroBlend"
    fi
    echo "All done with AstroBlend installation!"
fi



if [ $INST_BLENDER -eq 1 ]
then
    if [ -z "$BLENDER_DIR" ]
    then
	echo "BLENDER_DIR is not set, so I will be installing BLENDER"
	if [ "${MYOS##Darwin}" != "${MYOS}" ]
	then
	    echo "Installing for a Mac OSX (no other options yet!)"
	    $GETFILE http://ftp.halifax.rwth-aachen.de/blender/release/Blender2.72/blender-2.72a-OSX_10.6-x86_64.zip
	    unzip blender-2.72a-OSX_10.6-x86_64.zip
	    BLENDER_DIR=$DEST_DIR/Blender/blender.app/Contents/MacOS
	else
	    echo "Noooope.  No other options yet besides for Mac OS X. Nothing.  Nada.  Zip.  Exiting... sorry!"
	    exit 1
	fi
    else
	echo "BLENDER_DIR=${BLENDER_DIR} , so I will not be installing BLENDER"
    fi
    # now, run python to make special file
    echo "import sys" > $DEST_DIR/yt_blender_import.py
    echo 'vv = \' >> $DEST_DIR/yt_blender_import.py
    $DEST_DIR/bin/python$PYTHON_VER -c 'import sys; print(sys.path)' >> $DEST_DIR/yt_blender_import.py
    echo "for i in range(0,len(vv)):" >> $DEST_DIR/yt_blender_import.py
    echo "    if vv[i] not in sys.path:" >> $DEST_DIR/yt_blender_import.py
    echo "        sys.path.append(vv[i])" >> $DEST_DIR/yt_blender_import.py
    # now, make sure no python2 stuff is in that mess
    echo "vv2 = ['']" >> $DEST_DIR/yt_blender_import.py
    echo "for i in range(0,len(sys.path)):" >> $DEST_DIR/yt_blender_import.py
    echo "    if sys.path[i].find('python2') is -1:" >> $DEST_DIR/yt_blender_import.py
    echo "        vv2.append(sys.path[i])" >> $DEST_DIR/yt_blender_import.py
    echo "sys.path = vv2" >> $DEST_DIR/yt_blender_import.py
    # finally add in astroblend pointing
    echo "sys.path.append('"$ASTROBLEND_DIR"')" >> $DEST_DIR/yt_blender_import.py



    # now, update your bashrc file to add in this blender alias
    cd
    echo "alias blender='"${BLENDER_DIR}"/blender -P "${DEST_DIR}"/yt_blender_import.py'" >> $BASHFILE
    source $BASHFILE
    cd $DEST_DIR
    echo "You've just finished installing Blender & linking it to yt!  You ambitious code monkey you."
fi
