...
BLENDER_DIR="/Applications/Blender2.74/blender.app/Contents/MacOS"

# Here's where you put the AstroBlend path if you like; otherwise it'll download it
# and install it on its own
INST_ASTROBLEND=0 # install AstroBlend?
ASTROBLEND_DIR="/Users/jillnaiman/astroblend-dev/science/"

BASHFILE='.bash_profile'

BLENDER_COMMAND='blender2'
...
then
    ...
    $DEST_DIR/bin/$PYTHON_EXEC -c 'import sys; print(sys.path)' >> $DEST_DIR/yt_blender_import.py
    ...
    cd
    echo "alias "${BLENDER_COMMAND}"='"${BLENDER_DIR}"/blender -P "${DEST_DIR}"/yt_blender_import.py'" >> $BASHFILE
    echo "Blender command = "${BLENDER_COMMAND}
    source $BASHFILE
    cd $DEST_DIR
