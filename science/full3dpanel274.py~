...
# isocontour stuff
    ...
    bpy.types.Scene.isocontourValue = FloatProperty(
...
# for SlicePlot
    ...
    bpy.types.Scene.sliceCenterX = FloatProperty(
        name = "X", 
        description = "X Center",
        default = 0.0)

    bpy.types.Scene.sliceCenterY = FloatProperty(
        name = "Y", 
        description = "Y Center",
        default = 0.0)

    bpy.types.Scene.sliceCenterZ = FloatProperty(
        name = "Z", 
...
#    bpy.types.Scene.sliceNormalBool = BoolProperty(
#        name = "Use Normal", 
#        description = "True or False?")
#    scn['sliceNormalBool'] = False
...
#filename = "/Users/jillnaiman1/astroblend-dev/exampleScripts/full3dpanel.py"
#exec(compile(open(filename).read(), filename, 'exec'))


# AstroBlend utilities
class VIEW3D_PT_tools_ABObjects(View3DPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.label(text="LOAD FILE:") # right  now, just 1 file
        # example filename: /Users/jillnaiman1/data/IsolatedGalaxy/galaxy0030/galaxy0030
        layout.prop(scn, 'NewABFile')
        layout.operator("abload.button", text="Load Zee File")
...
#   AB Load Button
class ABLOAD_OT_Button(bpy.types.Operator):
    bl_idname = "abload.button"
    bl_label = "Button"

    def execute(self, context):
        from science import Load as LD
        myobj = LD(context.scene.NewABFile)#, context.scene.yt_object_name)
...
class VIEW3D_PT_tools_ABPlots_isocontour(View3DPanel, Panel):
    ...
    def draw(self, context):
        ...
        if obj:
            col = layout.column(align=True)
            #col.label(text="ISOCONTOUR:") # right  now, just 1 file
        ...
        for obj in bpy.data.objects:
            if obj:
                if obj.name == scn.sliceObjectName:
                    ...
                    row = col.row(align=True)
                    layout.prop(obj, 'location')
                    #layout.prop(scn, 'sliceNormalBool')

                    # also, update sphere location
                    #if scn.sliceNormalBool:
                    #    col.label(text="Normal:")
                    #    row = col.row(align=True)
                    #    layout.prop(bpy.data.objects['Empty'+scn.sliceObjectName], 'location')
                    ...
                    #elif scn.sliceAxis == 'z': # NOTE: this stuff might crap out...
...
class SLICE_OT_Button(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        fac = (ds.domain_width.in_units('unitary')[0]).value/(ds.domain_width.in_units(scn.sliceUnits)[0]).value
        bpy.data.objects[scn.sliceObjectName].scale = (scn.sliceWidth*fac,scn.sliceWidth*fac,scn.sliceWidth*fac)
        
        # now, derive the locations of the slices from this plane
        slcCenter = zeros(3)
        # example filename: /Users/jillnaiman1/data/IsolatedGalaxy/galaxy0030/galaxy0030
        for i in range(0,3): #need to recenter if object has moved
            newc = bpy.data.objects[scn.sliceLinkObjName].location[i] - ((ds.domain_center[i]).in_units('unitary')).value
            slcCenter[i] = bpy.data.objects[scn.sliceObjectName].location[i] - newc
        if scn.sliceAxis != 'off-axis':
            plots.ytSlicePlot(ds, image_name=scn.sliceObjectName, axis=scn.sliceAxis,
                              variable=scn.sliceVariable,
                              center=[slcCenter[0],slcCenter[1],slcCenter[2]],
                              width=scn.sliceWidth, units = scn.sliceUnits)
        else:
