...
for i in range(0,10):
    ...
    if i == 0: # for first, calculate all verticles
        ...
        for j in range(0,nfaces*4):
            print("j = " + str(j))
            if (j % 4) == 0: # if we are on a repeated face (new lower face) ("vertex 1")
                #vertices[:][vnum] = vertices[:][(vnum-nfaces*4) + j + 1]
                vertices[:][vnum+j] = vertices[:][(vnum-nfaces*4) + j]
                print("in first if")
            elif ((j+1) % 4) == 0: # if we are on a repeated face, 1 up ("vertex 4")
                #vertices[:][vnum+3] = vertices[:][(vnum-nfaces*4) + j - 1]
                vertices[:][vnum+3] = vertices[:][(vnum-nfaces*4) + j]
                print("in 2nd if")
            elif ((j-1) % 4) == 0: # if "vertex 2"
                rotation_angle_start = 360.0/nfaces*(j%4)
                matrix = rotation_matrix(rotation_angle_start, u1)
                # rotate uperp1 around u1 vector
                rv = np.dot(matrix,uperp1)
                # formatting
                ...
                # moar formating
                v1[np.abs(v1) < 1e-10] = 0.0
                vertices[:][vnum+1] = v1/(v1[0]**2.+v1[1]**2.+v1[2]**2)**0.5*r_cyl + line_pts[i+1]
                print("in 3rd f")
            elif ((j+2) % 4) == 0: # "vertex 3"
                rotation_angle_end = 360.0/nfaces*(j%4+1.0)
                matrix = rotation_matrix(rotation_angle_end, u1)
                # rotate uperp1 around u1 vector
                rv = np.dot(matrix,uperp1)
                # formatting
                ...
                v1[np.abs(v1) < 1e-10] = 0.0
                # 3rd matrix for 3rd vertex
                vertices[:][vnum+2] = v1/(v1[0]**2.+v1[1]**2.+v1[2]**2)**0.5*r_cyl + line_pts[i+1]
                print("in 4th if")
            vnum = vnum+4
...
meshname = 'mymesh'
#meshlocation = (vertices['x'][0], vertices['y'][0], vertices['z'][0])
meshlocation = (0, 0, 0)
plot_index = 0
